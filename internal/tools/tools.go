...
// HandleFindFunction handles the find_function tool call.
// Queries functions by exact name within a repository.
func (svc *Manager) HandleFindFunction(ctx context.Context, repo, name string) (*SearchResponse, error) {
	if repo == "" {
...
// HandleFindFile handles the find_file tool call.
// Glob matches file paths within a repository.
func (svc *Manager) HandleFindFile(ctx context.Context, repo, pattern string) (*SearchResponse, error) {
	if repo == "" {
		return nil, errors.New("repository is required")
	}
	if pattern == "" {
	...
	if len(matched) > 0 && len(matched) <= 5 {
		for i := range matched {
			symbols := svc.fetchSymbolsForFile(ctx, repo, matched[i].Path)
			if len(symbols) > 0 {
...
func (svc *Manager) fetchSymbolsForFile(ctx context.Context, repo, path string) []string {
	c, err := svc.db.ForRepo(ctx, repo)
	if err != nil {
		return nil
	}
...
// HandleSearchCode handles the search_code tool call.
// Classifies query, dispatches to appropriate strategy.
func (svc *Manager) HandleSearchCode(ctx context.Context, repo, query string, limit int) (*SearchResponse, error) {
	if repo == "" {
		return nil, errors.New("repository is required")
	}
	if query == "" {
	...
	switch strat {
	case strategyFile:
		return svc.HandleFindFile(ctx, repo, query)

	case strategyFuzzy:
		return svc.executeFuzzySearch(ctx, repo, query, limit)

	case strategyExact:
		resp, err := svc.HandleFindFunction(ctx, repo, query)
		if err != nil {
			return nil, err
		}
		...
		return svc.executeExactSupplement(ctx, repo, query, limit)
...
func (svc *Manager) executeExactSupplement(ctx context.Context, repo, query string, limit int) (*SearchResponse, error) {
	var results []SearchResult
	tokens := strings.Fields(query)
	for _, token := range tokens {
		resp, err := svc.HandleFindFunction(ctx, repo, token)
		if err != nil {
			continue
		}
...
func (svc *Manager) requireRepoClient(ctx context.Context, repo string) (*client.Client, error) {
	c, err := svc.db.ForRepo(ctx, repo)
	if err != nil {
