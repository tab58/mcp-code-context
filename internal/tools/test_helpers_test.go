...
// newTestService creates a Service backed by a noop driver.
func newTestService(t *testing.T) *Manager {
	t.Helper()
	db := newTestDB(t)
	return &Manager{db: db}
}

// newTestServiceWithIndexer creates a Service backed by a noop driver with an Indexer wired up.
// func newTestServiceWithIndexer(t *testing.T) *Service {
// 	t.Helper()
// 	db := newTestDB(t)
// 	idx := indexer.NewIndexer(db)
// 	return &Service{db: db, idx: idx}
// }

// newTestServiceWithResponses creates a Service backed by a response driver.
func newTestServiceWithResponses(t *testing.T, readResponses []driver.Result) (*Manager, *responseDriver) {
	t.Helper()
	db, drv := newTestDBWithResponses(t, readResponses)
	return &Manager{db: db}, drv
}
