...
// // HandleIngestRepository handles the ingest_repository tool call.
// // Runs the full pipeline (index -> analyze) on a local directory path.
// func (svc *Service) HandleIngestRepository(ctx context.Context, repoPath string) (*IngestResponse, error) {
// 	if repoPath == "" {
...
// 	// Step 1: Index
// 	result, err := svc.idx.IndexRepository(ctx, repoPath, indexer.WithProgress(func(_, _ string) {}))
// 	if err != nil {
...
// 	if svc.analyzer != nil && len(result.FilePaths) > 0 {
// 		analyzeResult, analyzeErr := svc.analyzer.Analyze(ctx, result.RepoID, repoPath, result.FilePaths, analysis.WithAnalyzeProgress(func(_, _ string) {}))
// 		if analyzeErr != nil {
...
// 	if svc.analyzer != nil && len(result.FilePaths) > 0 {
// 		if err := svc.analyzer.ComputeComplexity(ctx, result.RepoID, repoPath, result.FilePaths); err != nil {
...
// HandleDeleteRepository handles the delete_repository tool call.
// Removes all graph data for a repository.
func (svc *Manager) HandleDeleteRepository(ctx context.Context, repo string) (*DeleteResponse, error) {
	if repo == "" {
		return nil, errors.New("repository is required")
	}

	if err := svc.db.DeleteRepo(ctx, repo); err != nil {
...
// HandleGetRepositoryStats handles the get_repository_stats tool call.
// Returns node counts per type for a repository.
func (svc *Manager) HandleGetRepositoryStats(ctx context.Context, repo string) (*RepoStatsResponse, error) {
	if repo == "" {
		return nil, errors.New("repository is required")
	}
