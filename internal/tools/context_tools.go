...
// HandleGetRepoMap handles the get_repo_map tool call.
func (svc *Manager) HandleGetRepoMap(ctx context.Context, repo string) (*RepoMapResponse, error) {
	if repo == "" {
...
// HandleGetFileOverview handles the get_file_overview tool call.
func (svc *Manager) HandleGetFileOverview(ctx context.Context, repo, path string) (*FileOverviewResponse, error) {
	if repo == "" {
		return nil, errors.New("repository is required")
	}
	if path == "" {
...
func (svc *Manager) HandleGetSymbolContext(ctx context.Context, repo, name string) (*SymbolContextResponse, error) {
	if repo == "" {
		return nil, errors.New("repository is required")
	}
	if name == "" {
	...
	if isFunction {
		...
		callees, err := svc.traverseHops(ctx, c, []string{name},
	...
	overview, err := svc.HandleGetFileOverview(ctx, repo, symbol.Path)
	if err == nil {
...
// HandleReadSource handles the read_source tool call.
func (svc *Manager) HandleReadSource(ctx context.Context, repo string, names []string) (*ReadSourceResponse, error) {
	if repo == "" {
		return nil, errors.New("repository is required")
	}
	if len(names) == 0 {
...
// parseOverviewItems extracts items from a GraphQL result data map.
